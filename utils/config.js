const SECTIONS = [
  {
    ...
    addText: '添加资产',
    items: [
      { key: 'cash', name: '现金', multi: false, fields: [{ key: 'value', label: '金额' }] },
      { key: 'bankCards', name: '银行卡', multi: true, limit: 10, fields: [{ key: 'balance', label: '余额' }] },
      { key: 'alipayAccounts', name: '支付宝', multi: true, limit: 5, fields: [{ key: 'total', label: '总额' }] },
      { key: 'wechatAccounts', name: '微信', multi: true, limit: 5, fields: [{ key: 'balance', label: '零钱' }, { key: 'changeFund', label: '零钱通' }] },
      { key: 'medicalInsurance', name: '医保账户', multi: false, fields: [{ key: 'value', label: '余额' }] },
      { key: 'housingFund', name: '公积金', multi: false, fields: [{ key: 'value', label: '余额' }] },
      { key: 'otherAssets', name: '其他流动资产', multi: true, limit: -1, fields: [{ key: 'balance', label: '金额' }] }
    ]
  },
  {
    id: 'fixedAssets',
    ...
    items: [
      { key: 'selfUse', name: '自住房', multi: false, fields: [{ key: 'value', label: '估值' }] },
      { key: 'investment', name: '投资房', multi: false, fields: [{ key: 'value', label: '估值' }] },
      { key: 'other', name: '其他固定资产', multi: false, fields: [{ key: 'value', label: '金额' }] }
    ]
  },
  {
    id: 'shortLiabilities',
    ...
    addText: '添加负债',
    items: [
      { key: 'creditCards', name: '信用卡待还', multi: true, limit: 10, fields: [{ key: 'balance', label: '本期待还' }] },
      { key: 'huabeiBaitiao', name: '花呗/白条', multi: false, fields: [{ key: 'value', label: '待还总额' }] },
      { key: 'otherShort', name: '其他短期借款', multi: true, limit: -1, fields: [{ key: 'balance', label: '剩余本金' }] }
    ]
  },
  {
    id: 'longLiabilities',
    ...
    itemsStartActive: true,
    items: [
      { key: 'mortgage', name: '房贷', multi: false, fields: [{ key: 'value', label: '剩余本金' }] },
      { key: 'carLoan', name: '车贷', multi: false, fields: [{ key: 'value', label: '剩余本金' }] },
      { key: 'otherLong', name: '其他长期贷款', multi: true, limit: -1, fields: [{ key: 'balance', label: '剩余本金' }] }
    ]
  }
]
...
const ITEM_MAP = {}
SECTIONS.forEach(s => {
  s.items.forEach(item => { ITEM_MAP[item.key] = item })
})
...
function calcItemTotal(snapshot, key) {
  ...
  if (key === 'wechatAccounts') {
    return arr.reduce((s, inst) => s + (parseFloat(inst.balance) || 0) + (parseFloat(inst.changeFund) || 0), 0)
  }
...
function calcTotalAssets(snapshot) {
  ...
  total += (a.wechatAccounts || []).reduce((s, inst) => s + (parseFloat(inst.balance) || 0) + (parseFloat(inst.changeFund) || 0), 0)
