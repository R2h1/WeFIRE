...
function calcMonthlySavings(snapshots) {
  ...
  const sorted = [...snapshots].sort((a, b) =>
    getSnapshotId(a).localeCompare(getSnapshotId(b)),
  );
  ...
  let monthCount =
    (parseInt(lastParts[0]) - parseInt(firstParts[0])) * 12 +
    (parseInt(lastParts[1]) - parseInt(firstParts[1]));
...
function calcSnapshotTotals(snapshot) {
  ...
  return {
    ...
    liabilities:
      config.calcTotalLiabilities(snapshot) || snapshot.liabilities || 0,
  };
}
